# Slap Battles Multi Macro v1.4.1 — Dokumentacja developerska
...
Plik: `SlapBattlesMultiMacro.ahk` (~3336 linii), AutoHotkey v2.0
...
```
Startup
  └─ Weryfikacja licencji (HTTP GET → GitHub Gist JSON)
  └─ LoadXxxINI() × 6
  └─ InitDebugCode()
...
| Zmienna | Typ | Opis |
|---------|-----|------|
| `APP_NAME` | String | Nazwa aplikacji: `"Slap Battles Multi Macro"` |
| `APP_VERSION` | String | Wersja: `"v1.4.1"` |
...
Pobiera `licenses.json` z GitHub Gist, parsuje JSON, zwraca status:
...
- Sprawdza czy `RobloxPlayerBeta.exe` jest aktywny (`WinExist`)
...
3. CheckLicenseFull(key, &aHWID)
   └─ HTTP GET → GitHub Gist raw licenses.json
...
- **Gra (`RobloxPlayerBeta.exe`)** — wykrywana przez `WinExist` do auto-pauzy
- **GitHub Gist** — hosting `licenses.json` (gist.githubusercontent.com)
